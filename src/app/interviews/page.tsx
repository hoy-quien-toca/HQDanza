    ...
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden bg-zinc-900 font-black">
      ...
      <header className="border-b-4 border-green-600 p-4 md:p-6 bg-zinc-950 sticky top-0 z-50 shadow-xl text-left">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-6 font-bold uppercase tracking-widest text-sm items-center">
            ...
            <Link href="/interviews" className="text-green-600 underline decoration-2 underline-offset-4 font-black">Entrevistas</Link>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex flex-col items-center justify-center space-y-8 md:hidden font-black text-center">
          ...
          <Link href="/interviews" onClick={() => setIsMenuOpen(false)} className="text-4xl font-franklin text-green-600 italic">Entrevistas</Link>
          <Link href="/contact" onClick={() => setIsMenuOpen(false)} className="text-4xl font-franklin text-white italic">Contacto</Link>
          <Link href="/submit" onClick={() => setIsMenuOpen(false)} className="text-3xl font-franklin border-4 border-green-600 text-green-600 px-8 py-4 rounded-full animate-pulse">Subir Fecha</Link>
        </div>
      )}

      <main className="max-w-6xl mx-auto p-4 md:p-6 space-y-12 relative z-10">
        <h2 className="text-4xl md:text-6xl font-franklin text-center py-10 border-b-8 border-green-600 text-white leading-none uppercase">Entrevistas</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-green-600/20 border-4 border-green-600 rounded-[40px] p-10 font-black">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left font-black">
            {interviews.map((interview) => (
              <Link 
                ...
                key={interview.id}
                className="group border-4 border-white p-4 bg-zinc-950/80 hover:border-green-600 transition-all shadow-[8px_8px_0px_0px_rgba(34,197,94,0.3)] rounded-[32px] block font-black"
              >
                ...
                <div className="space-y-3 font-black">
                  ...
                  <div className="flex justify-between items-center text-zinc-500 font-bold text-[10px] uppercase tracking-tighter font-black">
                    ...
                    {interview.author && <p className="text-green-600 font-black">Por: {interview.author}</p>}
                  </div>
                </div>
              </Link>
            ))}
