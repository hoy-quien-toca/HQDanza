...
export default function InterviewDetailClient({ 
  ...
  const shareOnWhatsApp = () => {
    ...
    const text = `Mira la entrevista que encontre en HQDanza: ${data?.title}\n\nLink: ${window.location.href}`;
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
  };

  if (loading && !interview) {
    return (
      <div className="min-h-screen bg-black text-orange-500 flex flex-col items-center justify-center p-6 text-center font-franklin">
  ...
  if (errorState && !interview) {
    return (
      <div className="min-h-screen bg-zinc-900 text-white flex flex-col items-center justify-center p-6 text-center font-black">
        <h1 className="text-6xl uppercase italic text-orange-500 mb-4 tracking-tighter font-franklin">404</h1>
        <p className="text-xl uppercase tracking-widest mb-8">No encontramos la entrevista.</p>
        <div className="flex flex-col gap-4 w-full max-w-xs">
           <button onClick={() => window.location.reload()} className="bg-orange-500 text-white py-3 rounded-full font-black uppercase text-sm border-2 border-white">Reintentar</button>
    ...
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden text-left bg-zinc-900 font-black">
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            <Link href="/" className="hover:scale-110 transition-transform cursor-pointer focus:outline-none">
               <Image src="/logo-nuevo.png" alt="Logo" width={50} height={50} className="border border-white rounded-xl shadow-lg" />
            </Link>
            <div>
              <h1 className="text-xl md:text-4xl font-franklin tracking-tighter text-orange-500 leading-none">HQDanza</h1>
              <p className="text-[8px] md:text-[10px] font-bold text-white uppercase tracking-widest mt-1">Portal de danza y entrevistas</p>
            </div>
          </div>
          ...
          <nav className="hidden md:flex gap-6 font-bold uppercase text-sm items-center">
            <Link href="/" className="hover:text-orange-500 transition-colors font-black">Fechas</Link>
            <Link href="/interviews" className="text-orange-500 underline underline-offset-4 font-black">Entrevistas</Link>
            <Link href="/contact" className="hover:text-orange-500 transition-colors font-black">Contacto</Link>
            <Link href="/submit" className="border-2 border-orange-500 text-orange-500 px-4 py-1 bg-black rounded-full animate-pulse font-black">Subir Fecha</Link>
          </nav>
        </div>
      </header>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex flex-col items-center justify-center space-y-8 md:hidden font-black">
          ...
          <Link href="/interviews" onClick={() => setIsMenuOpen(false)} className="text-4xl uppercase text-orange-500 italic font-franklin">Entrevistas</Link>
          <Link href="/contact" onClick={() => setIsMenuOpen(false)} className="text-4xl uppercase text-white italic font-franklin">Contacto</Link>
          <Link href="/submit" onClick={() => setIsMenuOpen(false)} className="text-3xl uppercase border-4 border-orange-500 text-orange-500 px-8 py-4 rounded-full animate-pulse font-franklin font-black">Subir Fecha</Link>
        </div>
      )}
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-6 py-10 relative z-10 font-black">
        <button 
          ...
          className="fixed bottom-6 right-6 md:absolute md:top-4 md:right-4 bg-orange-500 text-white w-14 h-14 md:w-16 md:h-16 flex items-center justify-center font-black text-3xl border-4 border-white rounded-full shadow-[0_0_20px_rgba(249, 115, 22,0.5)] z-[60]"
        >
          X
        </button>
        ...
        <article className="space-y-8">
          <div className="text-center space-y-4 font-black">
            <span className="bg-orange-500 text-white px-4 py-1 text-xs md:text-sm font-franklin italic rounded-full inline-block font-black">GRUPO: {activeInterview.band_name}</span>
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-4 pt-6 text-zinc-500 font-bold uppercase text-[10px] md:text-xs">
               ...
               {activeInterview.author && <p className="text-orange-500 italic font-black">Por: {activeInterview.author}</p>}
            </div>
          </div>
          ...
          <div className="pt-16 border-t-4 border-zinc-800 flex flex-col items-center gap-8 pb-32 font-black">
            <p className="text-zinc-500 uppercase text-xs italic font-black">Gracias por leer HQDanza</p>
            <button onClick={shareOnWhatsApp} className="bg-orange-500 text-white px-10 py-4 font-black uppercase text-lg hover:bg-white hover:text-orange-500 shadow-xl rounded-full border-4 border-black italic font-franklin transition-all">Compartir en WhatsApp</button>
          </div>
        </article>
      </main>
      ...
      {showLogoModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative max-w-lg w-full bg-zinc-900 border-8 border-white p-6 rounded-[50px] shadow-2xl text-center">
            <button onClick={() => setShowLogoModal(false)} className="absolute -top-4 -right-4 bg-orange-500 text-white w-12 h-12 font-black text-2xl border-4 border-white rounded-full">X</button>
            <Image src="/logo-nuevo.png" alt="Logo Grande" width={600} height={600} className="w-full h-auto rounded-[30px]" />
            <h3 className="text-3xl font-franklin text-orange-500 mt-4 leading-none">HQDanza</h3>
          </div>
        </div>
      )}
    </div>
