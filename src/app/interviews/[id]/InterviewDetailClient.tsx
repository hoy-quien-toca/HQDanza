...
export default function InterviewDetailClient({ 
  ...
  if (loading && !interview) {
    return (
      <div className="min-h-screen bg-black text-green-600 flex flex-col items-center justify-center p-6 text-center font-franklin">
  ...
  if (errorState && !interview) {
    return (
      <div className="min-h-screen bg-zinc-900 text-white flex flex-col items-center justify-center p-6 text-center font-black">
        <h1 className="text-6xl uppercase italic text-green-600 mb-4 tracking-tighter font-franklin">404</h1>
        <p className="text-xl uppercase tracking-widest mb-8">No encontramos la entrevista.</p>
        <div className="flex flex-col gap-4 w-full max-w-xs">
           <button onClick={() => window.location.reload()} className="bg-green-600 text-white py-3 rounded-full font-black uppercase text-sm border-2 border-white">Reintentar</button>
    ...
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden text-left bg-zinc-900 font-black">
      ...
      <header className="border-b-4 border-green-600 p-4 md:p-6 bg-zinc-950 sticky top-0 z-50 shadow-xl">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-6 font-bold uppercase text-sm items-center">
            <Link href="/" className="hover:text-green-600 transition-colors font-black">Fechas</Link>
            <Link href="/interviews" className="text-green-600 underline underline-offset-4 font-black">Entrevistas</Link>
            <Link href="/contact" className="hover:text-green-600 transition-colors font-black">Contacto</Link>
            <Link href="/submit" className="border-2 border-green-600 text-green-600 px-4 py-1 bg-black rounded-full animate-pulse font-black">Subir Fecha</Link>
          </nav>
        </div>
      </header>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex flex-col items-center justify-center space-y-8 md:hidden font-black">
          ...
          <Link href="/interviews" onClick={() => setIsMenuOpen(false)} className="text-4xl uppercase text-green-600 italic font-franklin">Entrevistas</Link>
          <Link href="/contact" onClick={() => setIsMenuOpen(false)} className="text-4xl uppercase text-white italic font-franklin">Contacto</Link>
          <Link href="/submit" onClick={() => setIsMenuOpen(false)} className="text-3xl uppercase border-4 border-green-600 text-green-600 px-8 py-4 rounded-full animate-pulse font-franklin font-black">Subir Fecha</Link>
        </div>
      )}
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-6 py-10 relative z-10 font-black">
        <button 
          ...
          className="fixed bottom-6 right-6 md:absolute md:top-4 md:right-4 bg-green-600 text-white w-14 h-14 md:w-16 md:h-16 flex items-center justify-center font-black text-3xl border-4 border-white rounded-full shadow-[0_0_20px_rgba(34,197,94,0.5)] z-[60]"
        >
          X
        </button>
        ...
        <article className="space-y-8">
          <div className="text-center space-y-4 font-black">
            <span className="bg-green-600 text-white px-4 py-1 text-xs md:text-sm font-franklin italic rounded-full inline-block font-black">GRUPO: {activeInterview.band_name}</span>
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-4 pt-6 text-zinc-500 font-bold uppercase text-[10px] md:text-xs">
               ...
               {activeInterview.author && <p className="text-green-600 italic font-black">Por: {activeInterview.author}</p>}
            </div>
          </div>
