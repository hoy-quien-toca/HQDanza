    ...
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden text-left bg-zinc-900 font-black">
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-6 space-y-8 md:space-y-12 relative z-10 font-black">
        ...
        <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
          <div className="flex-1 space-y-8 md:space-y-12">
            <section className="bg-green-600 text-white p-3 md:p-4 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 items-center font-black uppercase italic shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] rounded-2xl md:rounded-3xl">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-[8px] md:text-[10px] opacity-80 uppercase font-black">Edad</span>
                <select value={ageRating} onChange={(e) => { setAgeRating(e.target.value); applyFilters(allEvents, department, genre, e.target.value, priceType); }} className="bg-black text-white p-2 border-2 border-white focus:outline-none font-bold text-[10px] md:text-xs uppercase rounded-xl font-black">
                  <option value="">Todas</option>
                  {activeAgeRatings.map(a => <option key={a} value={a}>{a}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-[8px] md:text-[10px] opacity-80 uppercase font-black">Entrada</span>
                <select value={priceType} onChange={(e) => { setPriceType(e.target.value); applyFilters(allEvents, department, genre, ageRating, e.target.value); }} className="bg-black text-white p-2 border-2 border-white focus:outline-none font-bold text-[10px] md:text-xs uppercase rounded-xl font-black">
                  <option value="">Todas</option>
                  {activePriceTypes.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              <button onClick={() => {setDepartment(''); setGenre(''); setAgeRating(''); setPriceType(''); setEvents(allEvents);}} className="col-span-full text-[10px] underline hover:text-black font-black uppercase text-center font-franklin">Limpiar Filtros</button>
            </section>
              ...
              {loading ? (
                ...
                events.map((event) => (
                  <div key={event.id} onClick={() => setSelectedEvent(event)} className="border-4 border-white p-2 md:p-4 hover:translate-x-1 hover:-translate-y-1 transition-all bg-zinc-950 shadow-[6px_6px_0px_0px_rgba(34,197,94,0.5)] flex flex-row md:flex-col items-center md:items-stretch gap-3 md:gap-4 group/card relative overflow-hidden cursor-pointer rounded-[24px] md:rounded-[32px] h-36 md:h-auto font-black">
                    ...
                    <div className="w-24 h-24 md:w-full md:aspect-square bg-zinc-800 shrink-0 border-2 border-zinc-700 relative flex items-center justify-center overflow-hidden rounded-xl md:rounded-2xl shadow-[inset_0_0_15px_rgba(0,0,0,0.5)]">
                      {event.flyer_url ? <img src={event.flyer_url} alt="Flyer" className={`object-cover w-full h-full transition-all duration-500 ${(event.is_sold_out || event.is_suspended) ? 'grayscale blur-[1px]' : 'group-hover/card:scale-105'}`} /> : <div className="text-zinc-600 font-black italic uppercase text-center text-[10px] font-franklin">FLYER</div>}
                      {event.is_sold_out && <div className="absolute inset-0 bg-red-600/60 flex items-center justify-center text-white font-black uppercase text-[10px] rotate-12">AGOTADO</div>}
                      {event.is_suspended && <div className="absolute inset-0 bg-zinc-700/60 flex items-center justify-center text-white font-black uppercase text-[10px] rotate-12">SUSPENDIDO</div>}
                    </div>
        ...
        <footer className="border-t-4 border-zinc-800 pt-8 pb-16 text-center font-black">
          <p className="text-zinc-500 font-bold uppercase text-[10px] max-w-2xl mx-auto tracking-tighter leading-relaxed italic">AVISO: HQDanza NO VENDE ENTRADAS. SOMOS UNA PLATAFORMA INFORMATIVA. LA VENTA Y ORGANIZACIÓN ES RESPONSABILIDAD DE LOS ORGANIZADORES.</p>
        </footer>
      </main>
