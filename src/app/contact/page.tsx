    ...
    <div className="min-h-screen text-white font-sans relative overflow-x-hidden bg-zinc-900 font-black">
      ...
      <header className="border-b-4 border-green-600 p-4 md:p-6 bg-zinc-950 sticky top-0 z-50 shadow-xl">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <Link href="/" className="flex items-center gap-4">
            <Image src="/logo-praxis.png" alt="Logo Azul" width={50} height={50} className="border-2 border-white rounded-2xl" />
            <div>
          ...
          <nav className="hidden md:flex gap-6 font-bold uppercase tracking-widest text-sm items-center">
            <Link href="/" className="hover:text-green-600 font-black transition-colors">Fechas</Link>
            <Link href="/interviews" className="hover:text-green-600 font-black transition-colors">Entrevistas</Link>
            <Link href="/contact" className="text-green-600 underline decoration-2 underline-offset-4 font-black">Contacto</Link>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex flex-col items-center justify-center space-y-8 md:hidden text-center font-black">
          ...
          <Link href="/contact" onClick={() => setIsMenuOpen(false)} className="text-4xl font-franklin text-green-600 italic">Contacto</Link>
      ...
      <main className="max-w-2xl mx-auto p-4 md:p-12 relative z-10 font-black">
        {sent ? (
          <div className="border-8 border-white p-12 bg-zinc-950 shadow-[12px_12px_0px_0px_rgba(34,197,94,0.5)] rounded-[40px] text-center space-y-6 font-black">
            <h2 className="text-5xl md:text-7xl font-franklin uppercase text-green-600 leading-none">¡Enviado!</h2>
            <p className="text-xl font-bold uppercase tracking-widest italic font-black">Te responderemos pronto.</p>
            <Link href="/" className="inline-block bg-white text-black px-12 py-4 rounded-full font-black uppercase hover:bg-green-600 hover:text-white transition-all shadow-lg font-black">Volver</Link>
          </div>
        ) : (
          <div className="border-8 border-white p-8 md:p-12 bg-zinc-950 shadow-[12px_12px_0px_0px_rgba(34,197,94,0.5)] rounded-[40px] relative font-black">
            ...
            <h2 className="text-5xl md:text-7xl font-franklin uppercase text-green-600 mb-2 leading-none">Contacto</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 text-left font-black">
              <div className="space-y-2">
                <label className="block font-black uppercase text-xs tracking-widest text-green-600">Tu Nombre</label>
                <input required name="name" className="w-full bg-zinc-900 border-4 border-white p-4 rounded-3xl focus:border-green-600 outline-none text-white font-bold uppercase shadow-inner" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="block font-black uppercase text-xs tracking-widest text-green-600">Email</label>
                  <input required type="email" name="email" className="w-full bg-zinc-900 border-4 border-white p-4 rounded-3xl focus:border-green-600 outline-none text-white font-bold shadow-inner" />
                </div>
                <div className="space-y-2">
                  <label className="block font-black uppercase text-xs tracking-widest text-green-600">Celular / WhatsApp</label>
                  <input required type="tel" name="phone" placeholder="09X XXX XXX" className="w-full bg-zinc-900 border-4 border-white p-4 rounded-3xl focus:border-green-600 outline-none text-white font-bold shadow-inner" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="block font-black uppercase text-xs tracking-widest text-green-600">Mensaje</label>
                <textarea required name="message" rows={5} className="w-full bg-zinc-900 border-4 border-white p-4 rounded-[32px] focus:border-green-600 outline-none text-white font-bold uppercase shadow-inner" />
              </div>
              ...
              <div className="space-y-2 p-4 bg-black/40 rounded-3xl border-2 border-zinc-800 font-black">
                ...
                <input 
                  ...
                  className="w-full bg-zinc-900 border-2 border-white p-3 rounded-2xl outline-none text-green-600 font-black text-center text-xl shadow-inner" 
                />
              </div>

              <button disabled={loading} className="w-full bg-green-600 text-white font-black uppercase text-2xl py-5 rounded-full hover:bg-white hover:text-black transition-all border-4 border-white shadow-xl italic tracking-tighter disabled:opacity-50">
