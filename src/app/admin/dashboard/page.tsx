...
const GENEROS = ["CONTEMPORANEA", "DANZA CLASICA", "FLAMENCO", "FOLKLORE", "MALAMBO", "MILONGA", "TANGO", "OTROS"];
...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="min-h-screen bg-black text-orange-500 flex items-center justify-center font-black text-2xl uppercase italic">Cargando Admin...</div>;
    ...
    return (
      <div
        ...
        className={`border-4 p-2.5 sm:p-3 rounded-2xl ${
          isPending
            ? 'border-orange-500 bg-orange-950/40 admin-pending-card'
            : 'border-orange-500 bg-zinc-950/80'
        }`}
      >
        {isPending && (
          <p className="text-center text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-orange-500 animate-pulse mb-2">
            ⚠ Pendiente
          </p>
        )}
        <div className="flex gap-2 sm:gap-3">
          <img
            src={ev.flyer_url || '/logo-nuevo.png'}
            alt={ev.band_name}
          ...
          <div className="min-w-0 flex-1 flex flex-col gap-1 min-h-[8.05rem] sm:min-h-[8.4rem]">
            <div className="flex items-start justify-between gap-1 flex-shrink-0">
              ...
              <div className="flex flex-shrink-0 gap-0.5 sm:gap-1">
                ...
                <button type="button" onClick={() => supabase.from('events').update({ is_approved: !ev.is_approved }).eq('id', ev.id).then(() => fetchData())} className={`${adminBtn} border-white ${ev.is_approved ? 'bg-zinc-800' : 'bg-orange-500'}`}>{ev.is_approved ? 'BAJAR' : 'APROBAR'}</button>
                <button type="button" onClick={() => deleteEvent(ev.id)} className={`${adminBtn} bg-red-600 border-white`}>BORRAR</button>
              </div>
            </div>
            <p className="text-[10px] sm:text-xs font-bold text-orange-500 uppercase line-clamp-2 flex-shrink-0">
            ...
            <div className="flex flex-col gap-1 sm:gap-1.5 flex-1 justify-between min-h-0">
              <button
                ...
                className={`${adminBar} flex-1 ${ev.is_featured ? 'bg-orange-500 border-white text-white' : 'bg-transparent border-orange-500 text-orange-500'}`}
              >
                BANNER
              </button>
              <div className="grid grid-cols-3 gap-1 sm:gap-1.5 flex-1">
                ...
                <button type="button" onClick={() => updateEventTag(ev.id, 'SALIDA SEGURA', ev.suggestion_tag)} className={`${adminBar} h-full text-[8px] sm:text-[9px] ${ev.suggestion_tag === 'SALIDA SEGURA' ? 'bg-orange-500 border-white text-white' : 'border-orange-500 text-orange-500'}`}>SALIDA SEGURA</button>
              </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white p-3 sm:p-4 md:p-6 font-sans relative text-left overflow-x-hidden font-black">
      <header className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center mb-4 sm:mb-6 border-b-4 border-orange-500 pb-4 sm:pb-6 bg-zinc-950 p-3 sm:p-4 sticky top-0 z-50 gap-3">
        <div><h1 className="text-xl sm:text-3xl md:text-4xl font-black uppercase italic text-orange-500 leading-none">ADMINISTRADOR HQDanza</h1></div>
        <div className="flex gap-2 sm:gap-4"><button onClick={() => router.push('/')} className="flex-1 sm:flex-none bg-white text-black px-4 py-2 font-black uppercase text-xs rounded-full">Web</button><button onClick={() => supabase.auth.signOut().then(() => router.push('/admin'))} className="flex-1 sm:flex-none bg-orange-500 px-4 py-2 font-black uppercase text-xs rounded-full">Salir</button></div>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10">
        {/* IZQUIERDA: FECHAS */}
        <section className="space-y-4 sm:space-y-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 bg-zinc-950 p-3 sm:p-4 border-l-8 border-orange-500 rounded-r-2xl font-black">
             <h2 className="text-xl sm:text-2xl uppercase italic text-orange-500 font-franklin">Fechas</h2>
             <button onClick={() => setEditingEvent({ id: 'new', band_name: '', venue: '', address: '', city: '', department: 'MONTEVIDEO', date: '', time: '21:00', age_rating: 'ATP', description: '', is_approved: false, price_type: 'range', genre: 'ROCK', flyer_url: '', price_min: '', price_max: '', ticket_type: 'link', ticket_contact: '' })} className="w-full sm:w-auto bg-orange-500 text-white px-4 sm:px-6 py-2 text-[10px] font-black uppercase rounded-full border-2 border-white">+ NUEVA FECHA</button>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:gap-4 pb-6">
            {pendingEvents.length > 0 && (
              <div className="flex flex-col gap-4">
                <div className="border-2 border-orange-500 bg-orange-500/20 rounded-2xl px-4 py-2 text-center admin-pending-banner">
                  <p className="text-sm font-black uppercase text-orange-500 tracking-widest">
        ...
        <section className="space-y-8 sm:space-y-12">
          ...
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-xl sm:text-2xl uppercase italic text-orange-500 border-l-8 border-orange-500 pl-3 sm:pl-4 bg-zinc-950 py-2 font-franklin">Mensajes</h2>
            <div className="space-y-2 sm:space-y-3 font-black">
              {messages.map((m) => (
                <div key={m.id} className="border-2 p-2.5 sm:p-3 flex justify-between items-center gap-2 border-white bg-zinc-900 rounded-2xl">
                  <div onClick={() => setSelectedMessage(m)} className="cursor-pointer flex-1 min-w-0 truncate pr-2"><h3 className="uppercase text-[10px] sm:text-xs text-orange-500">{m.name}</h3><p className="text-[9px] sm:text-[10px] text-zinc-300 truncate">"{m.message}"</p></div>
          ...
          <div className="space-y-4 sm:space-y-6 border-t-4 border-zinc-800 pt-6 sm:pt-8">
            <h2 className="text-xl sm:text-2xl uppercase italic text-orange-500 border-l-8 border-orange-500 pl-3 sm:pl-4 bg-zinc-950 py-2 font-franklin">Publicidad</h2>
            <form onSubmit={handleSaveSponsor} className="bg-zinc-950 p-4 border-4 border-white space-y-4 rounded-[32px] font-black">
              ...
              <button type="submit" disabled={uploading} className="w-full bg-orange-500 py-2 text-xs border-2 border-white rounded-full font-black">GUARDAR</button>
            </form>
              ...
              {sponsors.map(sp => (
                <div
                  ...
                  className={`border-4 p-2.5 sm:p-3 flex gap-2 sm:gap-3 rounded-2xl ${
                    sp.is_active ? 'border-orange-500 bg-zinc-950' : 'border-zinc-800 opacity-60 bg-zinc-900'
                  }`}
                >
                  <img
                    src={sp.image_url || '/logo-nuevo.png'}
                    alt={sp.client_name}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] sm:text-xs font-bold text-orange-500 uppercase mt-0.5">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      ...
                      <button type="button" onClick={() => toggleSponsorStatus(sp.id, sp.is_active)} className={`${adminBtn} bg-orange-500 border-white`}>{sp.is_active ? 'PAUSA' : 'ACTIVO'}</button>
          ...
          {/* ENTREVISTAS */}
          <div className="space-y-4 sm:space-y-6 border-t-4 border-zinc-800 pt-6 sm:pt-8">
            <h2 className="text-xl sm:text-2xl uppercase italic text-orange-500 border-l-8 border-orange-500 pl-3 sm:pl-4 bg-zinc-950 py-2 font-franklin font-black">Entrevistas</h2>
            <form onSubmit={handleSaveInterview} className="bg-zinc-950 p-4 border-4 border-white space-y-4 rounded-[32px] font-black">
              <input placeholder="Grupo / Artista" className="w-full bg-black border-2 border-white p-2 uppercase text-xs rounded-xl" value={newInterview.band_name} onChange={e => setNewInterview({...newInterview, band_name: e.target.value})} required />
            ...
            <div className="space-y-2 sm:space-y-3">
              {interviews.map(int => (
                <div
                  ...
                  className={`border-4 p-2.5 sm:p-3 flex gap-2 sm:gap-3 rounded-2xl ${
                    int.is_active
                      ? 'border-orange-500 bg-zinc-950 shadow-md'
                      : 'border-zinc-800 opacity-50 bg-zinc-900 grayscale'
                  }`}
                >
                  <img
                    src={int.image_url || '/logo-nuevo.png'}
                    alt={int.title}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] sm:text-xs font-bold text-orange-500 uppercase mt-0.5 truncate">{int.band_name}</p>
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      ...
                      <button type="button" onClick={() => toggleInterviewStatus(int.id, int.is_active)} className={`${adminBtn} bg-orange-500 border-white`}>{int.is_active ? 'PAUSA' : 'ACTIVO'}</button>
      ...
      <style jsx global>{`
        ...
        @keyframes admin-pending-blink {
          ...
          50% {
            border-color: #ffffff;
            box-shadow: 0 0 28px rgba(249, 115, 22, 0.85);
          }
        }
        .admin-pending-card {
